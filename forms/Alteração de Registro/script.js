...
$(document).ready(function () {
   // Adiciona o listener para o campo de salário destino
   $("#salarioDestino").on("blur", function () {
      handlePromotionValidation();
   });
});
...
function handlePromotionValidation() {
   ...
   console.log("codCargoAtual:", codCargoAtual);
   const codCargoDestino = $("#codCargoDestino").val();
   console.log("codCargoDestino:", codCargoDestino);
   const salarioDestino = $("#salarioDestino").val();
   console.log("salarioDestino:", salarioDestino);
   const numeroMatricula = $("#numeroMatricula").val();
   console.log("numeroMatricula:", numeroMatricula);

   // Se não houver matrícula, não faz nada
   if (!numeroMatricula) {
      return;
   }

   // Verifica se houve alteração de cargo
   if (codCargoDestino && codCargoAtual !== codCargoDestino) {
      checkPromotionRules(numeroMatricula);
   } else if (salarioDestino) {
      // Se não houve alteração de cargo, verifica a alteração de salário
      checkSalaryChange(numeroMatricula, salarioDestino);
   }
}

/**
 * Verifica se o salário foi alterado em comparação com o salário atual do funcionário.
 * @param {string} numeroMatricula - A matrícula do funcionário.
 * @param {string} salarioDestino - O novo salário proposto.
 */
function checkSalaryChange(numeroMatricula, salarioDestino) {
   const constraints = [DatasetFactory.createConstraint("RA_MAT", numeroMatricula, numeroMatricula, ConstraintType.MUST)];
   DatasetFactory.getDataset("ds_gtb_jdbc_016_matricula", null, constraints, null, {
      ...
      error: function (err) {
         console.error("Erro ao buscar dados da matrícula: ", err);
      },
   });
}
...
function checkPromotionRules(numeroMatricula) {
   ...
   // Promises para as consultas aos datasets
   const admissionDatePromise = getAdmissionDate(numeroMatricula);
   const lastSalaryChangePromise = getLastSalaryChangeDate(numeroMatricula);

   Promise.all([admissionDatePromise, lastSalaryChangePromise])
      .then((results) => {
         const admissionDate = results[0];
         const lastSalaryChange = results[1];
      ...
      .catch((err) => {
         console.error("Erro ao verificar regras de promoção: ", err);
      });
}
 ...
 * @param {string} numeroMatricula - A matrícula do funcionário.
 * @returns {Promise<string|null>} - A data de admissão ou null.
 */
function getAdmissionDate(numeroMatricula) {
   return new Promise((resolve, reject) => {
      ...
      DatasetFactory.getDataset("ds_gtb_jdbc_016_matricula", null, constraints, null, {
         success: function (data) {
            if (data && data.values.length > 0) {
               resolve(data.values[0].RA_ADMISSA);
            } else {
               resolve(null);
            }
         },
 ...
 * @param {string} numeroMatricula - A matrícula do funcionário.
 * @returns {Promise<string|null>} - A data da última alteração ou null.
 */
function getLastSalaryChangeDate(numeroMatricula) {
   return new Promise((resolve, reject) => {
      const constraints = [DatasetFactory.createConstraint("R7_MAT", numeroMatricula, numeroMatricula, ConstraintType.MUST)];
      DatasetFactory.getDataset("ds_gtb_jdbc_016_alteracao_salarial", null, constraints, null, {
         success: function (data) {
            if (data && data.values.length > 0) {
               resolve(data.values[0].R7_DATA);
            } else {
               resolve(null);
            }
         },
...
function showRHValidationModal() {
   FLUIGC.modal(
      {
         title: "Atenção",
         content: "A solicitação será avaliada pelo RH.",
